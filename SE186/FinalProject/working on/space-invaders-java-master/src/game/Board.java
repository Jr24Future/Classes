...
public class Board extends JPanel implements Runnable {
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawString("Score: " + enemyWave.score().toString(), 300, 20);
    ...
    private void animationCycle() {
        if(enemyWave.getNumberOfEnemies()==0) {
            inGame=false;
            score = enemyWave.score() * lives;
            message = "You Won!  Final Score: " + score;
    ...
    private void collisionMissileEnemies() {
        ...
        if(player.getM().isVisible()) {
            for (Enemy2 enemy2 : enemyWave.getEnemies2())
                if(enemy2.isVisible() && player.getM().collisionWith(enemy2)) {
                    enemy2.explosion();
                    enemyWave.decreaseNumberOfEnemies();
                    player.getM().die();
                }
        ...
        if(player.getM().isVisible()) {
            for (Enemy3 enemy3 : enemyWave.getEnemies3())
                if(enemy3.isVisible() && player.getM().collisionWith(enemy3)) {
                    enemy3.explosion();
                    enemyWave.decreaseNumberOfEnemies();
                    player.getM().die();
                }
