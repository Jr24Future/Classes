...
/**
 * Set BNO055 to Magnetometer-Only mode
 */
/*
void imu_setMagnetometerOnly() {
    // Set the IMU to config mode first
    bno_write(BNO_MODE_R, BNO_MODE_CONFIG);

    // Set the IMU to Magnetometer-Only mode
    bno_write(BNO_MODE_R, BNO_MODE_MAGONLY);

    // Give some time for the change to take effect
    timer_waitMillis(20);  // Small delay to ensure the sensor changes to new mode
}
*/
